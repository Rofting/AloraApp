...
public class AssistantActivity extends AppCompatActivity {
    ...
    private static final String INSTRUCCIONES_ACCIONES =
            ...
            "- COMPLETADO: el paciente confirma que tomó la medicación, realizó la tarea, o dice que ya lo hizo\n" +
            "- POSPONER: el paciente no puede hacerlo ahora (ej: 'no estoy en casa', 'estoy acostada', 'ahora no puedo', 'en un rato')\n" +
            "- CREAR_RECORDATORIO:Título;HH:MM:00;DIAS: cuando el paciente pide crear o programar un recordatorio. " +
            "DIAS opciones: TODOS, LUNES, MARTES, MIERCOLES, JUEVES, VIERNES, SABADO, DOMINGO (separados por coma si son varios).\n" +
            "Ejemplo de accion para crear: CREAR_RECORDATORIO:Pastilla azul;10:00:00;LUNES,MIERCOLES,VIERNES\n" +
    ...
    private void procesarConInteligenciaArtificial(String textoPaciente) {
        ...
        String mensajeConContexto =
                ...
                "\nTítulo Alarma en Foco: " + (tituloRecordatorioActual.isEmpty() ? "Ninguno" : tituloRecordatorioActual) +
                INSTRUCCIONES_ACCIONES +
                "\nEntrada del paciente: " + textoPaciente;

        api.enviarMensajeIA("Bearer " + tokenManager.getToken(), idPaciente, new ApiService.ChatRequest(mensajeConContexto))
    ...
    private void parsearYCrearRecordatorio(String accion) {
        ...
        if (partes.length < 2) {
            Toast.makeText(this, "No pude interpretar el recordatorio, inténtalo de nuevo", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        api.createReminder("Bearer " + tokenManager.getToken(), idPaciente, nuevo).enqueue(new Callback<Reminder>() {
            ...
            public void onResponse(Call<Reminder> call, Response<Reminder> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    AlarmHelper.programarAlarma(AssistantActivity.this, guardado.getId(), idPaciente, titulo, hora);
                    Toast.makeText(AssistantActivity.this,
                            "Recordatorio \"" + titulo + "\" creado para las " + hora.substring(0, 5),
                            Toast.LENGTH_LONG).show();
                    cargarRecordatoriosParaLaIA();
                } else {
                    Toast.makeText(AssistantActivity.this, "Error al crear el recordatorio", Toast.LENGTH_SHORT).show();
                }
            }
            @Override public void onFailure(Call<Reminder> call, Throwable t) {
                Toast.makeText(AssistantActivity.this, "Sin conexión al crear el recordatorio", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void posponerRecordatorio(Long idAntiguo, String titulo) {
        ...
        api.createReminder("Bearer " + tokenManager.getToken(), idPaciente, nuevoRec).enqueue(new Callback<Reminder>() {
            ...
            public void onResponse(Call<Reminder> call, Response<Reminder> response) {
                if (response.isSuccessful() && response.body() != null) {
                    Reminder guardado = response.body();
                    AlarmHelper.programarAlarma(AssistantActivity.this, guardado.getId(), idPaciente, titulo, nuevaHora);
                    idRecordatorioActual = guardado.getId();
                    Toast.makeText(AssistantActivity.this,
                            "Pospuesto a las " + nuevaHora.substring(0, 5), Toast.LENGTH_LONG).show();
                    cargarRecordatoriosParaLaIA();
                } else {
                    Toast.makeText(AssistantActivity.this, "Error al posponer: " + response.code(), Toast.LENGTH_SHORT).show();
                }
            }
            @Override public void onFailure(Call<Reminder> call, Throwable t) {}
        });
    ...
    private void guardarEnBitacora(String registro) {
        if (idPaciente == -1) return;
        ApiService api = ApiClient.getClient().create(ApiService.class);
        api.createCareLog("Bearer " + tokenManager.getToken(), idPaciente, new CareLog("INTERACCIÓN IA", registro))
